...
class rendering_bop_dataset_back_front(Dataset):
    ...
    def __getitem__(self, index):
        ...
        t = np.array(info_['cam_t_m2c']).reshape((3, 1))/1000.0
        RT = [R, t]
    ...
    def modified_sym_Rt(self, rot_pose, tra_pose, model_info, e_rot=None):
        ...
        for sym in model_info['symmetries_discrete']:
            # [Fix] Handle dictionary format generated from continuous symmetries
            if isinstance(sym, dict):
                trans_disc.append(sym)
            else:
                # Handle standard flat list format from json
    ...
    def update_obj_id(self, obj_id, obj_path):
        ...
        '''
        print(f"DEBUG: Available keys in obj_info: {self.dataset_info['obj_info'].keys()}")
...
class train_bop_dataset_back_front(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'cam_R_m2c' in info_ and 'cam_t_m2c' in info_:
            ...
            cam_t_m2c = np.array(info_['cam_t_m2c']).reshape((3,1))/1000.0 # 注意单位换算
        else:
            cam_R_m2c = np.eye(3)
        ...
        rgb_c, mask_vis_c, GT_Front_raw, GT_Back_raw = self.preprocess(rgb_c, mask_vis_c, GT_Front_c, GT_Back_c)
        return rgb_c, mask_vis_c, GT_Front_raw, GT_Back_raw, Bbox, cam_K, cam_R_m2c, cam_t_m2c
    ...
    def preprocess(self, rgb_c, mask_vis_c, GT_Front, GT_Back):
        rgb_c_pil = Image.fromarray(np.uint8(rgb_c)).convert('RGB')
        mask_vis_c = mask_vis_c / 255.
        mask_vis_c = torch.from_numpy(mask_vis_c).type(torch.float)
        
        # 修改：简单地转为 Tensor 并归一化到 [0, 1] (如果原始是 0-255)
        # 假设 GT_Front 是 numpy array (H, W, 3)
        GT_Front = torch.from_numpy(GT_Front).type(torch.float).permute(2, 0, 1) 
        GT_Back = torch.from_numpy(GT_Back).type(torch.float).permute(2, 0, 1)
        
        return self.composed_transforms_img(rgb_c_pil), mask_vis_c, GT_Front, GT_Back
class test_bop_dataset_back_front(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'cam_R_m2c' in info_ and 'cam_t_m2c' in info_:
            cam_R_m2c = np.array(info_['cam_R_m2c']).reshape((3,3))
            cam_t_m2c = np.array(info_['cam_t_m2c']).reshape((3,1))/1000.0
        else:
            cam_R_m2c = np.eye(3)
            cam_t_m2c = np.zeros((3, 1))
        rgb = cv2.imread(info_['rgb'])
