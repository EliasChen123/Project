    ...
    def predict(self, cam_K, img, obj_ids, confidence_threshold=0.75, conf=0.75, iou=0.50, max_det=200, pnp_op='ransac+comb'):
        ...
        with torch.amp.autocast('cuda'):
            ...
            if self.show_op is not None:
                ...
                # =============== 修改开始 ===============
                # 修复: 如果列表为空(未检测到物体)，跳过 cat 和可视化，直接返回
                if len(crop_rgbs_l) == 0:
                    results_dict['time'] = t2 - t1
                    # 如果需要显示原图(没有框)，可以在这里处理，或者直接返回
                    if self.show_op:
                        # 确保 draw_image 存在（即使没有检测框 draw_annotations 也会返回原图）
                        if 'draw_image' not in locals():
                            # 如果上面 YOLO 也没检测到，draw_image 可能没定义，或者定义了但只是原图
                             draw_image = img # 使用原始 numpy 图像
                        
                        results_dict['show_2D_results'] = draw_image
                        # 6D 可视化结果设为 None 或原图，防止外部调用报错
                        results_dict['show_6D_vis0'] = None 
                        results_dict['show_6D_vis1'] = None
                        results_dict['show_6D_vis2'] = None
                    return results_dict
                # =============== 修改结束 ===============    
