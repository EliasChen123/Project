...
from EPro_PnP.rotation_conversions import matrix_to_quaternion, quaternion_to_matrix
...
class DeepLabV3(nn.Module):
    def __init__(self, num_classes, efficientnet_key=None, input_channels=3):
        ...
        # 增加 4 个通道用于预测前、后表面的 w2d (front_x, front_y, back_x, back_y)
...
class HccePose_Loss(nn.Module):
    ...
    def forward(self, pred_front, pred_back, pred_mask, gt_front, gt_back, gt_mask):
        ...
        # 将 gt_mask 扩展一个维度，变成 [B, 1, H, W]，用于后续的掩码乘法
        gt_mask_expanded = gt_mask.unsqueeze(1).clone().detach()
          
        Front_L1Loss_v_l = []
        Back_L1Loss_v_l = []
        
        for k in range(3):
            # == 修改：使用 gt_mask_expanded 计算 error ratio ==
            front_error_ratio = self.cal_error_ratio(pred_front[:, k*8:(k+1)*8], gt_front[:, k*8:(k+1)*8], gt_mask_expanded)
            ...
            if self.weight_front_error_ratio[k] is None:
                ...
                for i in range(pred_front[:, k*8:(k+1)*8].shape[1]):
                    self.Front_error_list[k][i].append(self.current_front_error_ratio[k][i].cpu().numpy())
                    self.weight_front_error_ratio[k][i] = float(np.mean(self.Front_error_list[k][i].data))
            back_error_ratio = self.cal_error_ratio(pred_back[:, k*8:(k+1)*8], gt_back[:, k*8:(k+1)*8], gt_mask_expanded)    
            ...
            if self.weight_back_error_ratio[k] is None:
                ...
                for i in range(pred_back[:, k*8:(k+1)*8].shape[1]):
                    self.Back_error_list[k][i].append(self.current_back_error_ratio[k][i].cpu().numpy())
                    self.weight_back_error_ratio[k][i] = float(np.mean(self.Back_error_list[k][i].data))
            ...
            # == 修改：将这里的 pred_mask.detach().clone() 替换为 gt_mask_expanded ==
            Front_L1Loss_v = self.Front_L1Loss(pred_front[:, k*8:(k+1)*8]*gt_mask_expanded,(gt_front[:, k*8:(k+1)*8] *2 -1)*gt_mask_expanded)
            ...
            Back_L1Loss_v = self.Back_L1Loss(pred_back[:, k*8:(k+1)*8]*gt_mask_expanded,(gt_back[:, k*8:(k+1)*8] *2 -1)*gt_mask_expanded)
        ...
        # # 调用 EProPnP 计算损失
        # # 需先将密集预测 reshape 供抽样使用 (B, H, W, C)
        # p_front = pred_front.permute(0, 2, 3, 1)
        # p_back = pred_back.permute(0, 2, 3, 1)

        # epropnp_loss = self.compute_epropnp_loss(
        #     p_front, p_back, pred_w2d_front, pred_w2d_back, 
        #     coord_2d, gt_mask, gt_pose, cam_K
        # )
        ...
        return {
            ...
            # 'EProPnP_loss': epropnp_loss # 添加进总 Loss 中优化
        }
...
class HccePose_BF_Net(nn.Module):
    def __init__(
        ...
        # 修改为: 48(前后向code) + 1(mask) + 2(w2d权重) = 51
        self.net = DeepLabV3(48 + 1 + 2,  efficientnet_key=efficientnet_key, input_channels=input_channels)
    ...
    def inference_batch(self, inputs, Bbox, thershold=0.5):
        ...
        pred_mask, pred_front_back_code, pred_w2d = self.net(inputs)
        pred_mask_prob = self.activation_function(pred_mask)
        pred_mask_bin = pred_mask_prob.clone()
        pred_mask_bin[pred_mask_bin > thershold] = 1.0
        pred_mask_bin[pred_mask_bin <= thershold] = 0.0
...
#     try:
#         # [修改] 增加调试打印
#         print(f"📂 Scanning checkpoint directory: {check_point_path}")
#         if not os.path.exists(check_point_path):
#              raise FileNotFoundError(f"Path does not exist: {check_point_path}")
...
def load_checkpoint(check_point_path, net : HccePose_BF_Net, optimizer=None, local_rank=0, CUDA_DEVICE='0'):
    ...
    try:
        ...
        files = os.listdir(check_point_path)
        ...
        # 过滤掉非权重文件 (如隐藏文件)
        # saved_ckpt = [f for f in files if os.path.isfile(os.path.join(check_point_path, f)) and 'step' in f]

        # if not saved_ckpt:
        #     raise ValueError(f"No valid checkpoint files found (must contain 'step'). Files: {files}")
        # 过滤掉非权重文件，允许包含 'step' 的文件或者纯数字命名的文件
        saved_ckpt = [f for f in files if os.path.isfile(os.path.join(check_point_path, f)) and ('step' in f or f.isdigit())]

        if not saved_ckpt:
            raise ValueError(f"No valid checkpoint files found. Files: {files}")

        # 原有逻辑：解析分数
        saved_ckpt_s = []
        for i in saved_ckpt:
            try:
                score = float(i.split('step')[0].replace('_', '.'))
                saved_ckpt_s.append(score)
            except:
                print(f"⚠️ Skipping invalid filename: {i}")
                saved_ckpt_s.append(-1.0)

        saved_ckpt_id = np.argmax(saved_ckpt_s)
        file_to_load = os.path.join(check_point_path, saved_ckpt[saved_ckpt_id])
        print(f"📥 Loading checkpoint: {file_to_load}")

        checkpoint = torch.load(file_to_load, map_location='cuda:'+CUDA_DEVICE)
        net.load_state_dict(checkpoint['model_state_dict'])
        if optimizer is not None:
            optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        best_score = checkpoint['best_score']
        iteration_step = checkpoint['iteration_step']
        keypoints_ = checkpoint.get('keypoints_', []) # 使用 get 防止报错
        print(f"✅ Checkpoint loaded successfully! Score: {best_score}")

    except Exception as e:
        if local_rank == 0:
            print(f"❌ Error loading checkpoint: {e}") # 打印具体错误
            import traceback
