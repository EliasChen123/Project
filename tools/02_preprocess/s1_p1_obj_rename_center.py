...
if __name__ == '__main__':
    ...
    INPUT_PLY = os.path.join(ROOT_DIR, "output", "trocar", "models", "trocar.ply")
    obj_id = 1
    OUTPUT_PLY = os.path.join(os.path.dirname(INPUT_PLY), 'obj_%s.ply'%str(obj_id).rjust(6, '0'))

    print('-' * 30)
    print(f"[Info] 正在读取文件: {os.path.abspath(INPUT_PLY)}")
    print(f"[Info] 计划输出路径: {os.path.abspath(OUTPUT_PLY)}")
    # Open the PLY file
    # 打开 PLY 文件
    mesh = ml.MeshSet()
    try:
        mesh.load_new_mesh(INPUT_PLY)
        print("[Info] 网格加载成功 (Mesh Loaded)") # <--- 新增打印
    except Exception as e:
        print(f"[Error] 无法加载文件，请检查路径! 错误信息: {e}")
        exit()
    ...
    # <--- 新增打印：显示计算出的中心点，确认是否有数值 --->
    print(f"[Info] 计算出的物体中心: {vertex_center}")
    ...
    # 判断物体模型是否包含纹理图
    # Check texture
    tex_num = mesh_c.texture_number()
    # <--- 新增打印：告知用户是否检测到了纹理 --->
    if tex_num > 0:
        print(f"[Info] 检测到 {tex_num} 个纹理，正在处理...")
        ...
        if not os.path.exists(INPUT_PLY.replace('.ply', '.png')):
            ...
            print(f"[Info] 纹理图片已复制到: {OUTPUT_PLY.replace('.ply', '.png')}")
    ...
    if mesh_c.texture_number() > 0:
        ...
        print("[Info] PLY 文件内的纹理路径已修正。")
    # <--- 新增打印：最终成功提示 --->
    print('-' * 30)
    print(f"[Success] 运行成功！新文件已生成于:\n{os.path.abspath(OUTPUT_PLY)}")
    print('-' * 30)
