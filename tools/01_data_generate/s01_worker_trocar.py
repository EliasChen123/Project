...
import bpy
# 生成放大版近距离trocar, 标注原点在trocar底面中心
# ========================================
# >>> 测试步骤 1: 检查对象层级与命名
# ========================================
# Imported 9 objects
...
TEXTURE_DIR = os.path.join(BASE_OUTPUT_DIR, "Textures")
OBJ_DIR = os.path.join(BASE_OUTPUT_DIR, "trocar")
MODELS_DIR = os.path.join(OBJ_DIR, "models")
BLEND_PATH = os.path.join(MODELS_DIR, "trocar.blend")

OUTPUT_DIR = os.path.join(OBJ_DIR, "bop_dataset_raw") #测试用
...
def sample_valid_camera_pose(max_tries=100):
    ...
    SAFE_LIMIT_OFFSET = 0.0012  # 1.5cm (原代码曾设为 0.035 导致出画)
    ...
    for _ in range(max_tries):
        ...
        ctheta = np.cos(np.radians(120)) # 80度视锥角的一半
...
min_vec = np.min(trocar_bbox, axis=0)
max_vec = np.max(trocar_bbox, axis=0)

eye_center = (min_vec + max_vec) / 2
obj_size = np.linalg.norm(max_vec - min_vec)
cam_radius_min = obj_size * 3.0
cam_radius_max = obj_size * 6.0

trocar_pos = np.mean(trocar_bbox, axis=0)
# target_vec = trocar_pos - eye_center
target_vec = eye_center - trocar_pos
